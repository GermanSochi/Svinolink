...
Минималистичный бот для чата: кидаете ссылку на **Instagram Reel или пост** → бот отвечает **видео** (reply) с подписью:

> `Svinolink любит донаты`

### Важные правила
- Бот реагирует **только** на Instagram (`/reel/`, `/reels/`, `/p/`).
- YouTube и прочие ссылки **игнорируются** (в Telegram у них и так есть превью).
- Сначала пишет «Сек...», при ошибке — текст с причиной.

### Mini App «⚙️ Тригеры» (в группе)

В групповом чате: кнопка **⚙️ Тригеры** или команда `/triggers` → открывается Mini App в стиле Svinolink:

- список встроенных и своих триггеров;
- добавление, правка, удаление;
- «1 раз в сутки»;
- **«Сохранить для всех в чате»** — настройки для всех участников группы.

Нужен HTTPS: на Render задайте `WEBHOOK_BASE_URL` (или `PUBLIC_BASE_URL`) = URL сервиса, например `https://svinolink-bot.onrender.com`. Локально — ngrok и `PUBLIC_BASE_URL=https://xxxx.ngrok-free.app`.

### Триггеры в группе (настраиваются)
Файл `triggers.json` (или команды / Mini App):
...
| `PUBLIC_BASE_URL` | то же, если Mini App на другом домене (обычно можно не задавать) |
